...
public class CartController {
    ...
    @PostMapping("/{itemId}")
    public ResponseEntity<?> saveCart(@PathVariable("itemId") Long itemId) {
        cartService.saveCart(SecurityUtil.getCurrentName(),itemId);
    ...
    public ResponseEntity<List<CartResponse>> getAll() {
        return new ResponseEntity<>(cartService.getAll(SecurityUtil.getCurrentName()), HttpStatusCode.valueOf(200));
    }
}